...
public class BlockCornplant extends BlockCrops implements IGrowable {
	
	public static final PropertyInteger CORN_AGE = PropertyInteger.create("age", 0, 5);

	public static final AxisAlignedBB[] CORN_AABB = new AxisAlignedBB[] {
	...
	/**
	 * Constructor
	 * Registry name is set in the Registryhandler (see Modblocks class)
	 */
	...
	public void breakBlock(World world, BlockPos pos, IBlockState state) {
		...
		if(world.getBlockState(pos.down()).getBlock() == ModBlocks.CORNPLANT_MID){
			world.destroyBlock(pos.down(), true);
			world.destroyBlock(pos.down(2), true);
		}else
			if(world.getBlockState(pos.down()).getBlock() == ModBlocks.CORNPLANT){
				world.destroyBlock(pos.down(), true);
			}    
	}
	...
	public AxisAlignedBB getBoundingBox(IBlockState state, IBlockAccess source, BlockPos pos) {
		return CORN_AABB[state.getValue(CORN_AGE)];
	}
	
	@Override
	protected Item getCrop() {
		return ModItems.CORN_HORSE;
	}
	...
	public IBlockState getStateFromMeta(int meta) {
		...
		if (meta <= 7) return ModBlocks.CORNPLANT_MID.withAge(meta - 5);
		if (meta == 10) return ModBlocks.CORNPLANT_MID.withAge(3);

		if (meta == 8) return ModBlocks.CORNPLANT_TOP.withAge(0);
		if (meta == 11) return ModBlocks.CORNPLANT_TOP.withAge(1);
	...
	public IBlockState getNextState() {
		return ModBlocks.CORNPLANT_MID.getDefaultState();
	}
